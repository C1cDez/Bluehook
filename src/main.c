...
int execute(int argc, char** argv)
{
	...
	if (!strcmp("--radio", subcommand))
	{
		...
		if (argc >= 2)
		{
			...
			else if (contain_arg(argc, argv, "-nc") != -1)	radio_query.connectable = 2;
			if (contain_arg(argc, argv, "-d") != -1)		radio_query.discoverable = 1;
			else if (contain_arg(argc, argv, "-nd") != -1)	radio_query.discoverable = 2;
		}
	...
	else if (!strcmp("--pair", subcommand) || !strcmp("--auth", subcommand))
	{
		...
		int mitm_arg = contain_arg(argc, argv, "-m=");
		if (mitm_arg != -1)
		{
...
int main(int argc, char** argv)
{
	if ((argc == 2 && !strcmp("--help", argv[1])) || argc == 1)
	{
		printf(
			...
			"\t\t-c, -nc\t\t\tMake radio connectable/not-connectable\n"
			"\t\t-d, -nd\t\t\tMake radio discoverable/not-discoverable\n"
			"\n"
